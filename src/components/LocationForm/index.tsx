  ...
  return (
    <section>
      {showCoordsMap ? (
        ...
        <>
          {chooseDifferentLocation && (
            <div className={styles.choose_different_location}>
              <Button onClick={chooseDifferentLocation}>
                <MdArrowBack size="22px" />
                Choose Location
              </Button>
            </div>
          )}
          <form className={styles.form_container} onSubmit={preview}>
            ...
            {error && (
              <ErrorDisplay errorMessage={error} closeErrorDisplay={() => setError(false)} />
            )}
          </form>
        </>
      )}
